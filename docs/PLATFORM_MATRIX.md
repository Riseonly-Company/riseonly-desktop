...
| `window_chrome.rs` | Verified — policy tests plus a bundled window on macOS 26.1 | Compiled | Compiled | Every per-OS decision (control side, decoration mode, titlebar height, reserved inset, window-corner shape) is a pure function tested for all three. The macOS binding reaches GPUI's NSView, changes the owning NSWindow's WindowServer corner mask through private `_setCornerRadius:`, applies the same curve to GPUI's CAMetalLayer, then refreshes the native shadow; its 26.5pt continuous corner was pixel-compared with System Settings on the same 1x display (at most 0.2px half-edge error across rows 2–24). This is direct-distribution code, not Mac App Store-safe. Unverified elsewhere: that a Linux compositor actually grants the client-side decorations we request — which is why the request and the grant are separate types. |
